...
import de.hatoka.common.capi.entities.QuantityPO;
...
public class Quantity
{
    ...
    public static Quantity getInstance(QuantityPO quantity)
    {
        if (quantity == null)
        {
            return null;
        }
        return new Quantity(quantity.getAmount(), UnitScale.valueOf(quantity.getUnitScale()), Unit.valueOf(quantity
                        .getUnit()));
    }
    ...
    public BigDecimal divide(Quantity divisor)
    {
        ...
        {
            throw new IllegalArgumentException("Can't mix two different units " + unit.name() + " and "
                            + divisor.unit.name());
        }
        // 700 g div 1 kg
    ...
    public QuantityPO toQuantityPO()
    {
        return new QuantityPO(getAmount(), getUnit().name(), getScale().name());
    }
