...
public class TournamentListService extends AbstractService
{
    ...
    public Response create(@FormParam("name") String name, @FormParam("buyIn") String buyIn)
    {
        ...
        try
        {
            runInTransaction(() -> action.createTournament(name, new Date(), buyIn));
        }
        catch(Exception e)
        {
            render(e);
    ...
    {
        TournamentListAction action = getAction();
        runInTransaction(() -> action.deleteTournaments(identifiers));
        return redirect(METHOD_NAME_LIST);
    }
    ...
    {
        return getInstance(FrameRenderer.class).renderFame(content, "tournaments", subItem);
    }
}