<xsl:stylesheet version="2.0"
				...
				<nav class="navbar navbar-inverse navbar-fixed-top">
					<div class="container-fluid">
						...
						<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-9">
							...
							<xsl:if test="frameModel/@isLoggedIn = 'true'">
								<form action="{frameModel/@uriLogout}" class="navbar-form navbar-right">
									<div class="form-group">
										<button type="submit" class="btn btn-success glyphicon glyphicon-of"
											aria-hidden="true">Logout</button>
									</div>
								</form>
							</xsl:if>
							<xsl:if test="frameModel/@isLoggedIn = 'false'">
								<form action="{frameModel/@uriLogin}" class="navbar-form navbar-right">
									<div class="form-group">
	...
	<xsl:template match="message" xmlns="http://www.w3.org/1999/xhtml">
		<p class="bg-info">
			<xsl:value-of
				select="hatoka:getText($localizer, @messageKey, '', @parameter1, @parameter2, @parameter3)" />
		</p>
	</xsl:template>
	<!-- MAIN MENU TEMPLATE -->
